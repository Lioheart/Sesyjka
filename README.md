...
- naprawiono wyszukiwanie w rozwijanych polach formularzy przez jawne wyrażenie tekstowe Gtk.DropDown i dopasowanie fragmentu tekstu
- wszystkie główne tabele zapamiętują viewport i zaznaczenie przed otwarciem edytora lub operacją CRUD, dzięki czemu zapis nie przenosi widoku na początek listy
...
- przypisanie zasobu typu PDF do pozycji RPG automatycznie zaznacza format PDF w tej pozycji
- przypisanie zasobu typu VTT automatycznie zaznacza VTT. Jeżeli zasób podaje rozpoznawalną platformę, np. Foundry VTT lub Roll20, zostaje ona użyta jako nazwa platformy. Istniejąca ręcznie wpisana platforma VTT nie jest nadpisywana
- automatyczne oznaczanie formatów działa także przy skanowaniu lokalnych PDF i przy automatycznym powiązaniu zasobów DriveThruRPG
- odświeżanie tabel po dodaniu, edycji lub usunięciu rekordu zachowuje bieżące zaznaczenie, pozycję przewijania oraz stan rozwinięcia grup, dzięki czemu długa lista nie wraca do początku po zapisie
- bazy użytkownika pozostają domyślnie w `${XDG_DATA_HOME:-~/.local/share}/sesyjka/`. `SESYJKA_DATA_DIR` może wskazać inną lokalizację
...
- brak rekordu po stronie Supabase nie jest już interpretowany jako usunięcie. Usunięcie jest uznawane wyłącznie na podstawie jawnego tombstone `deleted=true`, co chroni lokalne dane przed niepełną odpowiedzią backendu lub ręcznym usunięciem wiersza w Supabase
- poprawiono kolejność stosowania rekordów `systemy_rpg` na podstawie rzeczywistej zależności `system_glowny_id`, aby rodzice byli tworzeni przed dziećmi, a przy usuwaniu dzieci znikały przed rodzicami
- przy błędzie klucza obcego synchronizacja podaje typ i identyfikator rekordu, a następnie wycofuje wszystkie lokalne zmiany wykonane w bieżącym przebiegu
- dodano konserwatywne odzyskiwanie `wydawcy.db`. Jeżeli baza wydawców jest pusta, ale kolekcja nadal zawiera identyfikatory wydawców, program przywraca najnowszą zgodną kopię zawierającą wszystkie używane identyfikatory. Mechanizm nie uruchamia się dla pustej, nieużywanej bazy
- zachowano zmianę z 0.9.9: `Nazwa` pokazuje produkt DriveThruRPG, a osobna kolumna `Nazwa pliku` pokazuje konkretny plik

## Zmiany w 0.9.9

- przywrócono nazwę produktu DriveThruRPG jako główną nazwę zasobu, np. wszystkie pliki produktu pozostają widoczne jako `Armie Apokalipsy`
- dodano do tabeli zasobów osobną kolumnę `Nazwa pliku`, która pokazuje przyjazny tytuł konkretnego pliku zwrócony przez DriveThruRPG, a gdy go brakuje, techniczną nazwę pliku
- techniczna nazwa pliku jest nadal przechowywana osobno. Dzięki temu Sesyjka nie traci informacji potrzebnej do rozpoznawania formatu i lokalnych plików
- baza `zasoby.db` otrzymuje automatyczną, zgodną wstecz migrację dodającą pole tytułu pliku. Ponowna synchronizacja DriveThruRPG uzupełnia je i przywraca nazwę produktu rekordom zaimportowanym przez 0.9.8
