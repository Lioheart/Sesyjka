...
| Obszar źródłowy | Stan portu GTK4 0.9.11 |
|---|---|
...
| Sesyjka Cloud | Osobna `sync.db`, Supabase Auth, okresowa synchronizacja offline-first, przyrostowe wysyłanie i pobieranie zmian, lokalny priorytet przy kolizji oraz synchronizacja ręczna bez zmian schematów baz użytkownika |
...
Wersja 0.9.11 nie manipuluje prywatnymi widgetami wierszy Gtk.ColumnView. Port zachowuje podstawowy przepływ pracy programu źródłowego: katalog kolekcji RPG, sesje, gracze, wydawcy, statystyki, filtry, transfer i bezpieczeństwo czterech baz. Wersja 0.9.11 obejmuje kolekcję planszówek i karcianek jako niezależne rozszerzenie w piątym pliku. Nie zmienia schematów czterech baz projektu źródłowego. Sesyjka Cloud dodaje wyłącznie osobną bazę `sync.db` z metadanymi synchronizacji, kolejką zmienionych baz i zgodnością ze starszymi konfliktami.


## Zasoby cyfrowe 0.9.11
